...
describe('handleAuthCreateSessionInnerGateway — deep-reach auth.log trace', () => {
  ...
  it("files the line under the network's own key, not whoever reached the box", async () => {
    ...
    // filed under each caller gives one path a row per visitor, and replay keeps only
    // whichever arrived last — the earlier player's reach disappears from the log.
    //
