/**
 * airmon-ng — toggle wireless monitor mode (the airodump-ng/aircrack-ng prerequisite).
 *
 * Ported from legacy `airmon-ng`, adapted to v2's connectivity model. Reads the
...
const execute: Command['execute'] = async (env, args) => {
  if (!isOwnWorkstation(env.session.machineId, env.identity.publicKeyHex)) {
    return error('airmon-ng: command not available on this machine');
  }
  ...
  if (name === undefined) {
    return error('airmon-ng: usage: airmon-ng <start|stop> <interface>');
  }

  const iface = env.network.interfaces().find((candidate) => candidate.name === name);
  if (iface === undefined) {
    return error(`airmon-ng: interface '${name}' not found`);
  }
  if (iface.kind !== 'wireless') {
    return error(`airmon-ng: ${name} is not a wireless interface`);
  }
  if (iface.association !== null) {
    return error(`airmon-ng: ${name} is already connected to a network`);
  }
  ...
  return error(`airmon-ng: unknown action '${action}' — use "start" or "stop"`);
};

export const airmonNg: Command = {
  name: 'airmon-ng',
  ...
  manual: {
    synopsis: 'airmon-ng <start|stop> <interface>',
    description:
      'Put a wireless interface into or out of monitor mode. Monitor mode is required before scanning for networks with airodump-ng or cracking handshakes with aircrack-ng.',
    arguments: [
    ...
    examples: [
      { command: 'airmon-ng start wlan0', description: 'Enable monitor mode on wlan0' },
      { command: 'airmon-ng stop wlan0', description: 'Disable monitor mode on wlan0' },
    ],
  },
  execute,
};
