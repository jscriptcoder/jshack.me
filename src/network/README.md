...
| `infraDaemonStateParser.ts` | Pure parser for infra daemon pid files (`/var/run/nginx.pid`, `/var/run/mysqld.pid`, `/var/run/redis.pid`, etc.) — content shape `${binary}:port=${N}` (multi-line for shared pid files). Reverse-derives pidFile→services from `INFRA_PID_CONFIGS`; port→service via canonical port table. Emits `InfraPortOverride[]`                           |
