...
Deterministic engine that generates a complete hackable network from a seed string. Same seed always produces identical output. No UI or React integration — pure generation pipeline.
...
2. **Topology** (`topology.ts`) — Flat subnet, machine count by difficulty, roles, IPs, interfaces, DNS
3. **Users** (`users.ts`) — Root + 1-2 role-appropriate users per machine, md5-hashed passwords
4. **Attack Chain** (`attackChain.ts`) — Path from entry to target, access methods, credential placements
5. **Filesystems** (`filesystem.ts`) — FileNode trees with role configs, credential breadcrumbs, noise, flag
...
| `types.ts`           | MissionNetwork, GeneratedMachine, AttackStep, MissionObjective |
| `pools.ts`           | Static data: usernames, passwords, hostnames, templates        |
| `topology.ts`        | Subnet generation, machine roles, NetworkConfig                |
