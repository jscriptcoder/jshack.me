...
| `systemLibraryLookup.ts` | `findLibraryCve(library, version, gameTime)`, `findLatestSafeLibrary(library, gameTime)`, `findPinnableLibraryVersion(library, version, gameTime)` — system library CVE lookup (libpam, libcrypt, libsystemd, libreadline, libssl, libz, libxml2, libpcre). Powers `msfconsole --local <command>` and `apt upgrade <library>`                                                                                                                                                                                                                                                                           |
...
| `timeline/`              | Procedural version timeline generator — deterministic CVE generation from service/firmware/library version templates. See [Timeline Pipeline](#timeline-pipeline) below                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `findLibraryCve(library, version, gameTime)` (`systemLibraryLookup.ts`) — shared system library CVE lookup (8 libraries). Also provides `findLatestSafeLibrary` and `findPinnableLibraryVersion`. Consumed by `msfconsole --local` (local exploitation) and `apt upgrade <library>` (defense).
- `findExploitableCve(machine, port, gameTime)` (`findExploitableCve.ts`) — top-level network exploit resolution used by msfconsole. Tries service CVE first, falls back to firmware CVE for routers. Library CVEs are routed via the separate `--local` path — they don't bind to a port.
...
| File                      | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `walker.ts`               | `buildTimelineFromTemplate` — walks a `VersionTemplate` forward with weighted-random bump types (80% patch, 15% minor, 5% major) and randomized day-gaps. Each `GeneratedVersion` also carries a `patchDelay` drawn from `[minPatchDelayDays, maxPatchDelayDays]` via a side-PRNG (keeps the main gap/bump stream stable). Also exports `buildTimeline` (service-specific wrapper), `findLatestSafeVersion`, `findGeneratedVersion` |
| `generatedVuln.ts`        | `buildGeneratedVuln(service, entry)` — constructs a deterministic `Vulnerability` from a walker entry (CVE id, severity, attack pattern, effect). CVE IDs are guaranteed unique across services + firmware + libraries via a deterministic (name, index) encoding                                                                                                                                                                   |
...
| `systemCommandEffects.ts` | Per-command effect pool for `msfconsole --local`. Library carries the CVE; command carries the semantics. One libpcre CVE → `dir_list` via `ls`, `file_read` via `grep`, `file_write` via `rm`. Tier = `root` for all pools (local exploitation typically lands at root)                                                                                                                                                            |
| `config.ts`               | `CVE_TIMING_CONFIG` (minSafeWindowDays: 3, maxSafeWindowDays: 14, minPatchDelayDays: 1, maxPatchDelayDays: 2), `DEFAULT_LATEST_VERSION`, `getLatestSafeVersion(service, gameTime)`, `assertCveTimingInvariants` (module-load guard: `minSafeWindowDays > maxPatchDelayDays`)                                                                                                                                                        |
...
| `pools/systemLibraryTemplates.ts` | `SystemLibrary` type, `systemLibraryTemplates` (8 libraries: libpam, libcrypt, libsystemd, libreadline, libssl, libz, libxml2, libpcre), `SYSTEM_LIBRARIES` list, `LIBRARY_META_PACKAGES` (auth-libs, crypto-libs, system-libs, data-libs) |
