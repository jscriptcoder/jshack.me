...
| `generatedVuln.ts` | `buildGeneratedVuln(service, entry)` — constructs a deterministic `Vulnerability` from a walker entry (CVE id, severity, attack pattern, effect). CVE IDs are guaranteed unique across services via a deterministic (service, index) encoding                                                                                                                                                                                       |
