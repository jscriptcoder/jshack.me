...
describe('generateHomeNetwork', () => {
  ...
  it('should have unique IPs across all machines', () => {
  ...
  it('should set localhostIp in the outermost subnet', () => {
    const network = generateHomeNetwork('test', 0, 'TEST');
  ...
  it('should have DNS records for layer machines', () => {
  ...
  it('should have layers based on difficulty', () => {
    // Generate many networks to cover different difficulties
    const layerCounts = new Set<number>();
    for (let i = 0; i < 30; i++) {
      const network = generateHomeNetwork(`layer-seed-${i}`, 0, 'TEST');
  ...
  it('should have machines with access variants', () => {
    const network = generateHomeNetwork('variant-test', 0, 'TEST');
    for (const machine of network.machines) {
  ...
  it('should have an entry variant and entry point', () => {
    const network = generateHomeNetwork('entry-test', 0, 'TEST');
  ...
  it('should have a routerMachine with users', () => {
    const network = generateHomeNetwork('router-test', 0, 'TEST');
  ...
  it('should alias inner gateway configs under downstream .1 IPs for multi-layer networks', () => {
    // Find a network with multiple layers
    for (let i = 0; i < 50; i++) {
      const network = generateHomeNetwork(`gateway-alias-${i}`, 0, 'TEST');
