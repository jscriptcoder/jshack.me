...
| lynx       | `lynx.ts`       | `lynx <url>`                             | Full-screen text-mode web browser (overlay). Requires `apt install lynx`. Arrow keys move between links, Enter follows, ← / Backspace pops history, q / Esc quits. HTML rendered with semantic markup; non-HTML served verbatim. Logs to target's `/var/log/access.log` like curl                                                                                                                                                                                |
...
| hydra      | `hydra.ts`      | `hydra <host> [service] [user]`          | Brute-force SSH/FTP logins, SNMP community strings, or MySQL credentials (async). Reads `/usr/share/wordlists/passwords.txt` — wordlist is the sole gate (deterministic). For FTP, uses virtual user creds when `/etc/vsftpd/virtual_users.conf` exists. Cross-player workstation ssh/ftp targets route through `/api/patches` `crackCredentials` (server-batched md5 matching, natural RTT pacing); local sweep applies for own / NPC / mission / world targets |
...
| `password_reset`     | none                       | Mutates `/etc/passwd` on the target, prints the new password. Cross-player-aware: reads via `exploitFileRead` at root tier and writes via `writeRemoteFile`. `effect.tier` selects which `/etc/passwd` line gets the new hash; the read/write themselves are always root-tier (the file is root-owned). Empty-hash lines (player's own user) get a non-empty hash written, _creating_ a password where none existed |
...
- `readRemoteFile(machineId, path, tier?)` — synchronous local read, used by `--local` dpkg parsing
...
- `exploitFileRead(machineId, path, tier)` — async cross-player-aware read for `file_read` CVE effect. On cross-player workstation targets, dispatches to `/api/patches` action `exploitRead` inside a `withTransientSession` (kind `effect_one_shot`); on NPC and own-workstation targets, falls back to local read.
