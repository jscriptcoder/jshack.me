...
| File                    | Description                                                                                                                                                                                                                                                                 |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `types.ts`              | zod schemas (createSession / endSession / listSessions / authCreateSession, action-discriminated), `SESSION_KINDS` enum, `AUTH_REQUIRED_KINDS` enum, `SessionRow`, `SessionSummary`.                                                                                        |
| `handler.ts`            | Single endpoint with action-dispatch: verify → rate-limit → branch into create/end/list/authCreate. Server-stamps `player_key`. `kind` is required (no default). Auth-required kinds (`ssh`/`scp`/`su`) on `createSession` return 403 — those must use `authCreateSession`. |
