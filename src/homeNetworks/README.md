...
| `homeNetworkHelpers.ts`   | All workstation-identity + cross-player addressing helpers in one file: `deriveHostnameSuffix` (8-hex), `computePlayerHostname`, `isOwnWorkstation`, `displayPromptHostname` (prompt-only suffix strip), `targetMachineIdFor` (LAN IP → canonical machine_id resolver: own-LAN → own hostname, gateway `.1` alias → gateway primary IP, LAN occupant → occupant hostname, else passthrough), `occupantAwareReadNode` (read-side symmetric wrap), `buildGatewayCanonicalIpMap` (HomeNetwork → `Map<aliasIp, canonicalIp>` for the gateway translation step), `buildResolveTargetMachineId` (shared composition consumed by `useNetworkCommands` + `Terminal.tsx`). |
