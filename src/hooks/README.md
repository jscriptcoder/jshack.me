...
| `useNetworkCommands.ts`    | Creates network commands (ifconfig, ping, nmap, nslookup, ssh, curl, ftp, nc, msfconsole, gobuster, hydra) with context from `useNetwork` and `useFileSystem`. Applies version overlays and wires game time into network commands. For hydra, dispatches cross-player workstation ssh/ftp targets through the server's batched `crackCredentials` endpoint; own-workstation / NPC / mission / world targets stay on the sync local /etc/passwd sweep |
...
| `exploitFileRead(machineId, path, tier)`           | Async cross-player-aware read for the `file_read` CVE effect. On cross-player workstation targets, dispatches to `/api/patches` action `exploitRead` inside a `withTransientSession`; on NPC and own-workstation targets, falls back to local read at tier. |
