...
| `fileSystemUtils.ts`    | Pure utility functions: path resolution (`normalizePath`, `resolvePath`), immutable tree operations (`getNodeAtPath`, `updateNodeAtPath`, `addChildAtPath`, `removeChildAtPath`), patch helpers (`upsertPatch`, `applyPatches`, `isValidPatch`). `checkTraversal` delegates to `permissionWalker` for the per-dir execute check.      |
...
| `FileSystemContext.tsx` | React context provider wiring `fileSystemUtils` with React state, IndexedDB persistence, and cross-tab sync. Leaf permission checks in `canReadFromMachine` / `canWriteFromMachine` delegate to the shared walker.                                                                                                                    |
