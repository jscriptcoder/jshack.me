...
| Home-network LANs    | Server-allocated slots + identity-derived hostnames | Shipped — `home_networks` + `home_network_occupants` tables, `POST /api/join-home-network` (signed, idempotent), random LAN slot in [.10, .250], `${prefix}-${4-hex-of-sha256(player_key)}` suffix |
...
- **Live cross-player updates via Realtime hint broadcast** — every successful `upsertPatch` / `removePatch` fan-outs a `patch_change` HINT (`{ machine_id, originator_key }`) on a `patches:<machine_id>` broadcast channel. Subscribers refetch authoritative state via the signed `listPatchesForMachines` endpoint (debounced ~150ms; pending local writes replayed on top). Forging the hint cannot inject content — there's nothing forge-able in the payload, and the actual data fetch goes through the signed endpoint. Hints whose `originator_key` matches the receiver's own pubkey are skipped (the local optimistic apply / cross-tab `BroadcastChannel` already covered the change). See `project_realtime_publish_authorization` memory.
