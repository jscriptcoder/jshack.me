...
**Source IP:** `resolveLogSourceIP()` in `src/logging/utils.ts` determines the correct source IP for log entries. When on a remote machine (SSH session), the remote machine's IP is used directly. When on the player's own workstation, every log line records the home router's public IP — a single identity per player that's consistent across every world-network and mission log. Falls back to the LAN IP only when no home network is connected. The own-workstation check uses `sessionMachine === ownWorkstationId` (post-PR-#94 workstations have identity-derived hostnames; the explicit comparison parameter prevents the literal-`'localhost'` regression).
