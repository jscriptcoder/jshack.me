...
**Source IP:** `resolveLogSourceIP()` in `src/logging/utils.ts` determines the correct source IP for log entries, mirroring real network behavior. When on a remote machine (SSH session), the remote machine's IP is used directly. When on the player's own workstation: same-/24 targets see the LAN IP (no NAT — same-LAN traffic stays on the LAN), and cross-network targets see the home router's public IP (NAT'd through the gateway). Falls back to LAN IP only when no home network is connected. The own-workstation check uses `sessionMachine === ownWorkstationId` (post-PR-#94 workstations have identity-derived hostnames; the explicit comparison parameter prevents the literal-`'localhost'` regression).
...
- **FTP** — two-stage login (username prompt → password prompt), resolves NAT. Server-authoritative as of PR 3 of cross-player-base-fs-replication: client calls `authCreateFtpSession` (action `authCreateSession`, kind `ftp`); server reads `/etc/vsftpd/virtual_users.conf` (overlay) and `/etc/passwd` (fallback) from `machine_filesystems` and validates credentials atomically with session creation. userType is server-derived from `/etc/passwd`. Triggers `onFtpAuth` callback for logging
