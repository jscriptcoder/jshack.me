...
- **Gateway .1 IP aliases** — inner gateway configs/filesystems are aliased under their downstream `.1` IPs so players can SSH into routers/gateways from inside the network. **Storage hygiene**: cross-player writes/reads addressed at a gateway's `.1` alias canonicalize to the gateway's primary IP via `targetMachineIdFor` (`src/homeNetworks/homeNetworkHelpers.ts`) — the home router is the load-bearing case (primary IP = public IP, distinct from `.1`). Without canonicalization, LAN-side writes would land under `machine_id="<subnet>.1"` while cross-LAN subscribers (who only know the primary IP) would query a different `patches` row and never observe the edit. Both the write path (`useNetworkCommands.logFs`) and the read paths (`occupantAwareReadNode`-wrapped readers in `NetworkContext`) share the same `buildResolveTargetMachineId` helper so writers and readers agree on the canonical machine_id.
...
**LAN-side vs WAN-side scans of a gateway** (PREROUTING semantic): real iptables NAT only fires on packets arriving on the WAN interface, so forwarded ports stay invisible from inside the LAN. `applyDynamicOverrides` (`src/network/networkUtils.ts`) detects the LAN-side view by comparing the visible IP to its canonical-key resolution — when they differ (i.e., the viewer is addressing a gateway's `.1` alias), the gateway-NAT-merge step is skipped. The router's own port state (daemon-state from pid files, SNMP firewall overrides, ACL filtering) stays symmetric across both interfaces — only the forward merge is gated. Defenders can hide what's forwarded from intruders with only LAN foothold; pivoting onto the router via SSH+iptables read becomes a meaningful unlock.

**Home-router forwarding to workstation occupants**: a player who hacks root on their own home router can edit `/etc/iptables/rules.v4` to forward the router's public IP to their workstation (e.g. expose a player-run `apache2` / `nginx` on the LAN to anyone scanning the router's public IP). `buildMergedRouterView` resolves forward rules against two pools: `internalMachines` (NPC home/mission machines) AND `occupantMachines` (LAN-occupant workstations, port state derived from pid-file overlay). NPC entries win on IP collision. Workstation accounts are intentionally NOT merged into the router's user list — they live behind the per-player `/etc/passwd` projection, not the router. Mission and world routers don't share a LAN with the player, so `buildRouterRemoteView` (the wrapper used for those) deliberately doesn't take an occupant parameter.

Wiring path: `NetworkContext` builds `overlaidOccupants` once per render by applying `applyDynamicOverrides` to each occupant (their gateway-NAT branch is a no-op since workstations aren't gateways; the daemon-state branch reads their pid files). The overlaid occupants flow into `DynamicOverrideContext.overlaidOccupants`, which `applyDynamicOverrides` passes to `buildMergedRouterView` when the home router is being overlaid. Same-LAN scope only — foreign-LAN home routers aren't reachable today (planned as a separate piece 2b alongside lazy subscription).
...
- `apache2:port=N,user=X,userType=T,home=P` — `/var/run/apache2.pid` (consumed by `parseApache2State`). Owner is the invoking user, not a hardcoded www-data. Port→service: 443→https, 8080→http-alt, else→http.
- `${binary}:port=${N}` — every infra pid file in the **short form** (themed-network generators, consumed by `parseInfraDaemonState`). Multi-line content is supported: nginx serving both 80 and 443 ships a single `nginx.pid` with two lines (`/usr/sbin/nginx:port=80\n/usr/sbin/nginx:port=443`). Services sharing a pid file (http/https/http-alt → nginx.pid; imap/imaps/pop3 → dovecot.pid) share fate — if the pid file is missing, all of them close. **Exception**: services actively served by a running apache2 are excluded from the nginx.pid-absent closure so apache2-opened http/https doesn't get closed when nginx isn't running on the same machine.
- `${binary}:port=${N},user=X,userType=T,home=P` — every infra pid file in the **extended form** (player-run `nginx` command, consumed by the same parser). Owner is the invoking user. Same parser, optional trailing fields.

**Player-driven daemon control**: the corresponding command writes the pid file at start (`sshd [port]`, `vsftpd [port]`, `apache2 [port]`, `nginx [port]`, `nc -l <port>`, or `bash('/usr/sbin/<daemon>')` from an NC shell). `sshd`/`vsftpd`/`systemctl` are root-only admin utilities pre-installed in `/usr/sbin/` on every machine; `apache2`/`nginx` are apt-installable packages (`apt install apache2` / `apt install nginx`) that land in `/usr/bin/` and enforce a privileged-port (<1024) gate requiring root. `systemctl stop <service>` deletes the pid file to close the port (currently `sshd`/`vsftpd` only — `apache2`/`nginx` direct-command shutdown is a follow-up).
