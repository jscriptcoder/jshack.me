...
**Player-driven daemon control**: the corresponding command writes the pid file at start (`sshd [port]`, `vsftpd [port]`, `apache2 [port]`, `nginx [port]`, `nc -l <port>`, or `bash('/usr/sbin/<daemon>')` from an NC shell). `sshd`/`vsftpd`/`systemctl` are root-only admin utilities; `apache2`/`nginx` are open to any user but enforce a privileged-port (<1024) gate that requires root. `systemctl stop <service>` deletes the pid file to close the port (currently `sshd`/`vsftpd` only — `apache2`/`nginx` direct-command shutdown is a follow-up).
