...
- **Completion** — Player sends proof via `mail("client@darkmail.onion", "proof")`. Three objective types: exfiltrate (find ACCESS-KEY, optionally encrypted — requires `decrypt(file, key)` as root), tamper (modify a file), credential_theft (steal root password). The `mail` command verifies proof and calls `completeMission()`.
