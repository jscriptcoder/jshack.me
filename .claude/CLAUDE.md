...
- Ten objectives: exfiltrate, tamper, credential_theft, script_fix, script_auto, sabotage, backdoor, portforward, forensics, malware
...
- **Basic SNMP on gateways**: Non-SNMP-variant inner gateways have a difficulty-based PRNG chance (easy 70%, medium 40%, hard 20%) of having basic SNMP enabled. Within basic SNMP, a second PRNG roll (~30%) determines read-write vs read-only. **Basic read-only**: `rocommunity public` only — interface OIDs for subnet discovery. **Basic read-write**: rw community + firewall/ACL OIDs (no credential leaks) — players can `snmpset` to open firewalled ports. SSH daemon runs behind the firewall (PID file exists). Full SNMP configs (SNMP-variant gateways) also include credential leaks. Both PRNG rolls are always consumed per gateway for sequence stability.
