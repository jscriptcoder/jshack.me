...
- **Apt-installable tools** in `/usr/bin/` — require `apt install` as root (needs network); only WiFi tools (airmon, airdump, aircrack), node, and gpg are pre-installed on localhost; world-executable once installed (except `gpg`: root-only). Includes `redis-tools` package (installs `rediscli`)
...
`rediscli(host[, password])` connects to a remote machine's Redis server (port 6379). Requires `apt install redis-tools`. Enters an interactive `redis>` prompt with command parsing:

- **Supported**: `KEYS pattern`, `GET key`, `SET key value`, `DEL key`, `DBSIZE`, `AUTH password`, `QUIT`/`EXIT`
- **Data model**: Key-value store per machine stored as `/var/lib/redis/data.json`. ~35% of database machines have Redis open (port 6379)
...
- `/var/log/redis.log` — Redis connections (Redis log format)
