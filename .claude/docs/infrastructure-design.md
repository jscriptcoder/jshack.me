...
Each crackable WiFi network maps to a unique "home network" — a seeded subnet of machines generated by `generateHomeNetwork(seed, index, essid, usedIps?)` in `src/generation/generateHomeNetwork.ts`. Each home network contains:

- **1 router** with a unique public IP (collision-aware via `usedIps`) and dual interfaces
