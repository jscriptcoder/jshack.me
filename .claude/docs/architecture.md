...
│   └── generateMission.ts     # Mission orchestrator: seed → MissionNetwork (uses generateNetwork)
...
**Pipeline**: `generateMissionNetwork(seed, usedIps?)` uses the shared `generateNetwork()` pipeline (`generateNetwork.ts`) for topology → users → enrichment → port closures → config updates → base filesystems, then layers on mission-specific steps: attack chain (`attackChain.ts`) → objective filesystems (`filesystem.ts`) → binary wrapping (`binary.ts`). Home networks use the same shared pipeline via `generateHomeNetwork()`. Seeds can embed keywords to override generation axes — see `parseSeedOverrides()` in `generateMission.ts`. Shared IP utilities (`ip.ts`) provide `generatePublicIp(prng, usedIps?)` and `generatePrivateSubnet(prng)` — used by both mission and home network generation. When `usedIps` is provided, public IP generation re-rolls to avoid collisions.
