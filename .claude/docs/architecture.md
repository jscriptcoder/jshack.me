...
- **script_fix** — Player finds a broken script on the target machine, fixes it with `nano()`, and runs it with `node()`. Scripts call `_submit()` on success — a function injected only into `node()`'s execution context during script_fix missions. It auto-completes the mission (no `mail()` step). The ACCESS-KEY never appears in the script source (anti-cheat). Bug types: syntax (missing paren/quote), logic (wrong comparison), corrupted (data replaced with `???`, hint file nearby). ~60% user-owned (anyone can edit/run), ~40% root-owned (must `su` first).
