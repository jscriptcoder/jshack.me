...
**Status**: 5b.1a ✅ (#307). 5b.1b-i ✅ (#308, v0.73.0). 5b.1b-ii ✅ (#309, v0.74.0). 5b.2 ✅ (#310, v0.75.0). 5b.3a ✅ (#311, v0.76.0). 5b.3b ✅ (#312, v0.77.0). 5b.4 (CHAINS) sub-split a/b/c/d — **5b.4a ✅ (#313, v0.78.0); 5b.4b ✅ (#314, v0.79.0); 5b.4c-i ✅ (#315, v0.80.0); 5b.4c-ii ✅ (#316, v0.81.0); 5b.4d ✅ MERGED (#317, v0.82.0, squash `f98f5ab`)**. **5b.4 (CHAINS) COMPLETE.** 5b.5 GRILLED 2026-06-25 (D-trace-1..5), sub-split a/b/c — **5b.5a ✅ MERGED (#318, v0.83.0, squash `12e2e03`); 5b.5b ✅ (v0.84.0, branch `feat/v2-5b-5b-deep-scan-trace`)**. Next = **5b.5c (octet reservation + close-out)**.

> **Status: Slice 5b.5b ✅ DONE (v0.84.0). 5b.5 sub-split a/b/c — NEXT = Slice 5b.5c. ⟵
> RESUME (exact pick-up): a pivot `nmap` of a deep `/24` now fires a fire-and-forget `env.scan.recordDeep`
> (signed `nmapScanDeep` on `/api/patches`) that lands ONE aggregate `kern.log` line on each touched deep host
> (terminal NPC + child gateway when in range), sourced from the fronting gateway's `${deep.subnet}.1`, writer =
> the caller's own key. 5b.5b as-built: NEW shared `core/scan/deepScanHosts.ts` (`resolveDeepScanHosts` →
> `{subnet, sourceIp, hosts:[{host, machineId, ports}]}`) is consumed by BOTH the client pivot render and the
> server trace, so display + trace can't drift (it also FIXED a latent bug: a switch CHILD gateway sibling now
> resolves through its switch base FS via `resolveDeepGatewayIdentity` instead of aliasing the NPC tree). NEW
> server handler `core/scan/nmapScanDeep.ts` re-derives the vantage from the verified key + `vantage_machine_id`
> via the NEW consolidated `chainGatewayVantageForMachineId` (one chain walk → `{...PivotVantage, baseFs}`; a
> forged/non-gateway vantage logs nothing), materializes a SWITCH vantage's journal to read its live `acl.conf`
> (a router skips the journal), and appends per touched host via `appendMachineLog` + `formatNmapScanAggregate`.
> New seam `env.scan.recordDeep` → `recordDeepScan` adapter (`patchApi.ts`) → `onScanRecordDeep` (`env.ts`/
> `state.ts`); `mockScanApi` gained `recordDeep`. `api/patches` gained the `nmapScanDeep` dispatch block.
> Mutation: `nmapScanDeep.ts` 100%, `deepScanHosts.ts` 96% (lone survivor = the switch-discriminant `true`
> variant, provably equivalent — a router base FS has no `acl.conf`), changed `lanHostIdentity` region 100%,
> `nmap.ts` region 1 documented-unreachable down-host default. Wire-check `scripts/testDeepScanTrace.ts` 6/6
> (router NPC + child from the gateway `.1`; switch deep NPC; a live `deny 22` filters `:22` and removing it
> re-opens it — also kills the deferred deep-switch `acl.conf`-seed survivor end-to-end; forged edge-`.1`
> vantage writes nothing).
>
> Start **5b.5c (octet reservation + close-out)** — firmed decisions D-trace-1..5 + 5b.5c AC are in the
> "Slice 5b.5" section below. 5b.5c shape: `mergeLanOccupants` gateway-wins (protect generated `kind !==
> 'machine'` hosts — drop a colliding occupant from THIS viewer's nmap); then fold the 5b epic-reconciliation
> notes into `multiplayer-crossplayer-epic.md`, delete this plan, and bump the conventions-doc status. NO branch
> cut yet for 5b.5c; AC awaiting the RED gate. The 5b.4d/5b.4c-ii as-built detail is below.**
>
...
- [ ] **5b.5 (TRACES + octet reservation — grilled 2026-06-25 → D-trace-1..5)** A deep-layer scan/connect
  ...
  - [x] **5b.5b** Deep SCAN trace — a pivot `nmap` fires a fire-and-forget server trace that appends
        `kern.log` per touched deep host (shared resolution; switch records post-ACL visible ports). _(v0.84.0)_
...
**Resolved decisions (grill-me, 2026-06-25) — D-kind-1..5:**
- **D-kind-1 — kind seeding + placement:** each deep child gateway's kind is seeded by a NEW
  `seedDeepGatewayKind(key, parentMid, octet)` on the `deep-gw-kind-${key}:${parentMid}:${octet}` PRNG
  namespace (mirrors `seedDeepGatewayAdminPw`): `switch` iff `next() < DEEP_SWITCH_PROBABILITY` (= `0.33`),
  else `router`. Applied at EVERY deep child slot, INCLUDING the inner router's first child. The new namespace
  leaves every existing seed (`deep-layer-`, `deep-gw-admin-`, `inner-gw-*`) byte-stable — only the chain SHAPE
  changes. `seedNetworkDepth` becomes a MAX (longest possible chain), not an exact length.
- **D-kind-2 — a switch truncates the chain:** a switch forwards nothing, so it is a chain LEAF. This already
  falls out of the existing code: `generateDeepLayer` only hangs a `childGateway` when
  `frontingGateway.kind === 'router'`, so the moment a child is seeded `switch`, `chainFrom` stops there. A
  switch seeded mid-chain ends the chain early (effective length = `min(seedNetworkDepth, position-of-first-
  switch)`); a switch at the terminal slot is just an ACL-flavored leaf.
- **D-kind-3 — switch fronts its OWN ACL-scannable NPC layer:** a deep switch still fronts a deep NPC `/24`
  (`generateDeepLayer` for a switch fronting gateway yields `host` = NPC, `childGateway: null` — already the
  code's shape). The NPC is pivot-SCANNABLE (ACL-filtered by the switch's `acl.conf`, the 5b.3b mechanic one+
  layer deeper) but RECON-ONLY — there is no forward path through a switch, so the NPC is never reachable.
  Matches the L1 switch semantics.
- **D-kind-4 — deep-switch FS + credential:** add `buildDeepSwitchBaseFs(key, parentMid, octet)` =
  `buildGatewayBaseFs({ adminPwHash: md5(seedDeepGatewayAdminPw(key, parentMid, octet)), hasSsh: true },
  { switch: acl.conf (ACL_CONF_SEED) })` — REUSES `seedDeepGatewayAdminPw` (the slot is one kind, so the
  shared `deep-gw-admin-` namespace is unambiguous; mirrors how the L1 switch reuses `inner-gw-admin-`).
  `machineId` stays `computeDeepGatewayId(key, parentMid, octet)` (kind-AGNOSTIC). `resolveDeepGatewayIdentity`
  gains a `kind` param and branches ONLY the FS builder (`buildDeepSwitchBaseFs` vs `buildDeepGatewayBaseFs`).
- **D-kind-5 — wire-check:** re-pick `alice` in `testDeepChainReach.ts` to `seedNetworkDepth === 3` AND
  all-router (existing R→R→R reach + brick-darkens-below assertions stay green, now robust to kind-seeding);
  ADD a sibling `scripts/testDeepSwitchChain.ts` for a router→switch identity.

**Forced reach/scan semantics (recorded; fall out of D-kind-4 once `resolveDeepGatewayIdentity` is kind-aware):**
- reach: the parent router forwards `<pub> → <switchIp>:22` → lands ON the switch (auth its admin pw); a
  forward to `<switchIp>:<non-22>` is dark (the switch has no other open port — no NAT involved).
- upstream scan of a switch child surfaces ONLY its `:22` → `resolveHostPorts` in `resolveDeepPivotScan`
  widens `host.kind === 'router'` to `host.kind !== 'machine'` (a router OR switch child resolves via
  `resolveDeepGatewayIdentity(..., host.kind)`; an NPC stays the `buildDeepHostFs` branch).
- pivot + L2-write from a rooted deep switch work for FREE once `resolveDeepGatewayIdentity` is kind-aware:
  `chainFrom` passes `child.kind` so the `ChainVantage.baseFs` is the switch FS → `chainGatewayBaseFsForMachineId`
  resolves it for the L2 `nano acl.conf` write, and `pivotVantageForMachineId` already returns `kind: 'switch'`
  so `resolveDeepPivotScan` ACL-filters. `resolveChildGatewayHop` threads `kind` from `deep.childGateway.kind`.
...
**Acceptance criteria** (FIRMED via grill-me 2026-06-25 → D-kind-1..5):
- [ ] A deep child gateway's kind (`router`/`switch`) is seeded deterministically from `(key, parentMid,
  octet)`, reload-stable, with every existing router seed byte-unshifted (new `deep-gw-kind-` namespace).
- [ ] A switch deep gateway hangs NO child gateway (chain truncates there) but DOES front its own deep NPC
  `/24`; `seedNetworkDepth` reads as a max chain length.
- [ ] A deep switch is reachable through its parent's forward to `:22` (auth its `seedDeepGatewayAdminPw`
  credential); a forward to any non-22 port on it is dark; an upstream scan of it surfaces only `:22`.
- [ ] From a rooted deep switch, a pivot `nmap` of its deep `/24` is ACL-filtered by its `acl.conf`, and
  `nano`-deleting a `deny` line re-opens that port on the next scan (L2 write to the switch's acl.conf).
- [ ] `testDeepChainReach.ts` stays green (alice re-picked depth-3 all-router); a new
...
### Slice 5b.5 — Deep-layer traces + octet reservation + close-out (GRILLED 2026-06-25 → D-trace-1..5)

Recon leaves a readable trace on deep boxes; the inner gateway survives a colliding occupant; the story
is closed out. Sub-split **a/b/c** (walking-skeleton-first: thin connect trace → the bigger scan trace →
octet + close-out).

**Grilled decisions:**

- **D-trace-1 — trace BOTH deep scan AND deep connect (full fidelity).** Depth is single-player/private,
  so a deep trace is a SELF-trace (read only by the same player, after they breach the box). Still worth
  full fidelity: the deep net is the pivot-practice surface (D5), and a playground where recon leaves no
  trace teaches the wrong lesson and denies track-covering practice. The pivot scan RESULT stays
  client-side (deterministic, fast); only a fire-and-forget side-effect trace call is added.
- **D-trace-2 — source IP = the fronting gateway's `<deep subnet>.1` ("came from the gateway"), for BOTH
  scan and connect.** Unified rule: a trace logs the box the attacker operates FROM, as routable on the
  target's segment. An outside cross-player scan logs the scanner's home public IP (shipped, UNCHANGED);
  an inside deep scan/connect logs the vantage gateway's downstream `.1`. Server-computable by
  regenerating the deep layer from the verified key + vantage `machine_id` → `${deep.subnet}.1`; no
  registry lookup. This is NOT the deferred cross-player masking-half (pivoting through ANOTHER player's
  box to hide your IP from a THIRD party, D4) — the deep case is your own private chain, so logging the
  gateway is just correct, not gold-plating.
- **D-trace-3 — scan trace = client result + fire-and-forget server side-effect.** Mirrors the shipped
  own-LAN scan (`nmap.ts` `void env.scan.record(...)`). The pivot result keeps resolving CLIENT-side; a
  new vantage-keyed server action regenerates the deep layer and appends ONE aggregate `kern.log` line
  per touched deep host (terminal NPC + child gateway if in range) via `appendMachineLog` +
  `formatNmapScanAggregate`. A switch vantage records the post-ACL VISIBLE ports (what actually reached
  the host). The deep host/port resolution that today lives client-side in `resolveDeepPivotScan` is
  EXTRACTED to a shared `core/` fn consumed by BOTH the client render and the server trace, so they
  can't drift.
- **D-trace-4 — connect trace = auth.log on the existing reach path.** `authCreateSessionInnerGateway`
  gains an `auth.log` append on the landed deep box: `Accepted password … from <gateway .1>` on success,
  `Failed password …` on a wrong-password reach (both readable once you're IN — the failure line surfaces
  earlier attempts after you eventually get in). A no-forward/dead-port reach writes nothing (no target
  box to log on). NO new api action — rides the existing server round-trip.
- **D-trace-5 — octet reservation = gateway wins in the merge.** `mergeLanOccupants` protects generated
  `kind !== 'machine'` hosts (the inner router + switch): a colliding occupant is dropped from THIS
  viewer's nmap (still in `home_network_occupants` + attackable via their public IP). The non-colliding
  case is byte-unchanged (pure addition on the collision path). Priority: your private depth-entry beats
  one occupant's same-LAN visibility. The collision is structural (the occupant's octet is
...
gateway.
**Path**: `ssh user@<inner>:<fwd>` → `authCreateSessionInnerGateway` lands the session on the deep box →
the server appends an `auth.log` line (`formatSshdAuthLine`, source = `${deep.subnet}.1` derived by
regenerating the deep layer) to the landed `machine_id` via `appendMachineLog` (writer = the verified
caller key, per-viewer) → after landing, `cat /var/log/auth.log` (journal-replayed) shows it. A
wrong-password reach appends a `Failed password` line to the same box; a no-forward/dead-port reach
writes nothing.
**Required implementation skills**: `tdd`, `testing`, `mutation-testing`, `refactoring`.
**Acceptance criteria** (confirm before code):
1. A successful deep reach appends `Accepted password for <user> from ${deep.subnet}.1` to the landed
   deep box's `/var/log/auth.log`, readable after landing (journal-replayed).
2. A wrong-password reach against a RESOLVABLE deep box appends `Failed password for <user> from
   ${deep.subnet}.1` to that box; a no-forward/dead-port reach writes NO line.
3. The source IP is the fronting gateway's downstream `.1` of the deep subnet (server-derived), never the
   player's home IP.
4. Best-effort: a logging failure never fails the reach (the session still lands).
5. The shipped reach behaviour stays green; a wire-check (extend `testDeepChainReach.ts` /
   `testInnerGatewayReach.ts`) drives the real `/api/sessions` reach and asserts the auth.log line.
**RED/GREEN/MUTATE/KILL/REFACTOR**: per the skills. **Done when**: criteria met, ~100% mutation on changed
core, wire-check green, version bumped, human approves.

#### Slice 5b.5b — Deep scan trace (fire-and-forget server side-effect)

**Value**: A pivot `nmap` of a deep `/24` leaves a `kern.log` line on each touched deep host — the recon
is recorded, readable after you breach the host; the practice surface logs scans like a real network.
**Path**: pivot `nmap <deep /24>` resolves the RESULT client-side (unchanged) AND fires a
fire-and-forget `env.scan.recordDeep({ essid, target, vantage })` → a new server action regenerates the
deep layer from the verified key + vantage `machine_id` + kind, resolves the touched hosts via the SHARED
`core/` resolution, and appends one aggregate `kern.log` line (source = `${deep.subnet}.1`) per host via
`appendMachineLog`. A switch vantage records the post-ACL visible ports. Best-effort.
**Required implementation skills**: `tdd`, `testing`, `mutation-testing`, `refactoring`.
**Acceptance criteria** (confirm before code):
1. A deep pivot scan appends an aggregate `kern.log` line to each touched deep host (terminal NPC + child
   gateway when in range), readable on that host after breach (journal-replayed), source =
   `${deep.subnet}.1`.
2. The trace records the SAME ports the pivot scan displays (the deep host/port resolution is shared, so
   render and trace can't drift) — a behavioural test exercising both proves the equivalence.
3. A switch vantage records the post-ACL visible ports: a `deny`'d port does NOT appear in the trace, and
   deleting the `deny` makes it reappear on the next scan (this also kills the deferred 5b.4d
   deep-switch `acl.conf`-seed survivor).
4. Fire-and-forget: a trace/round-trip failure never breaks or delays the scan RESULT.
5. The pivot scan RESULT is byte-unchanged from today (client-resolved); a new wire-check (extend
   `testDeepSwitchChain.ts` or a sibling `testDeepScanTrace.ts`) drives the real api action and asserts
   the lines.
**RED/GREEN/MUTATE/KILL/REFACTOR**: per the skills. **Done when**: criteria met, ~100% mutation on changed
core, wire-check green, version bumped, human approves.

#### Slice 5b.5c — Octet reservation + story close-out

**Value**: A busy AP can no longer eat your inner gateway/switch — your depth entry always survives; the
story is closed out.
**Path**: `mergeLanOccupants` drops a colliding occupant when its octet matches a generated
`kind !== 'machine'` host (the gateway/switch survive); then fold the 5b epic-reconciliation notes back
into `multiplayer-crossplayer-epic.md`, delete this plan, and bump the conventions-doc status.
**Required implementation skills**: `tdd`, `testing`, `mutation-testing`, `refactoring`.
**Acceptance criteria** (confirm before code):
1. When a real occupant's octet equals the viewer's inner gateway or switch octet, the merged LAN keeps
   the gateway/switch and OMITS that occupant; the non-colliding case is unchanged (every occupant still
   wins over a generated `machine` sibling).
2. Close-out: `multiplayer-crossplayer-epic.md` updated (5b absorbed the reachability half; cross-player
   source-IP masking still deferred; deep traces now log the gateway); plan deleted; conventions-doc
   status → 5b.5 done.
**RED/GREEN/MUTATE/KILL/REFACTOR**: per the skills. **Done when**: criteria met, ~100% mutation on the
merge change, human approves.
