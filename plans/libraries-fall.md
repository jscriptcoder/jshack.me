...
63–73 (decision 69 supersedes 17). Prior slice close-out: "Phase 3 slice 7 — reboot evicts"
(#515–#518, v0.231.0–v0.234.0), which closed V3. **Slice 9 (firmware) is the only V-series axis
left after this.**

**Status:** Active — PR1 next, not yet branched. Nothing merged.

**Delivery:** Five independent PRs, sequenced to trunk (NOT a stack), per decision 72. Each merges
to `main`; the next branches from updated `main`. PRs 1 and 2 are independent of everything and of
each other; PR3 needs PR2 only for the "guest carries the tool in" story (it runs unaided on a box
the player already holds root on); PR4 needs PR3's interpreter; PR5 widens PR3's map. Versions
0.235.0 → 0.239.0, bumped in both `v2/package.json` and `v2/package-lock.json` per PR.

**Branches (proposed):** PR1 `feat/ldd-lists-linked-libraries`, PR2 `feat/run-a-carried-binary`,
PR3 `feat/local-exploit-escalates`, PR4 `feat/local-exploit-crosses-players`,
PR5 `feat/library-map-covers-the-toolchain`.
...
|---|---|
| 9 | Severity → tier, with libraries floored at user (`critical`/`high` → root, `medium`/`low` → user). The interpreter PR3 writes reads this floor; `exploitEffect.ts`'s `tierForSeverity` is the service floor and must not be reused unchanged for libraries. |
| 11 | `libraryDeps` is the single authority for what a command links; `metadata.libraryLinks` is deleted (PR1). |
| 12 | The map extends to the game's own tools, keeping eight libraries; each added command needs its own pool (PR5). |
...
All eleven (63–73) plus the seven find-gaps answers live in the epic's `### Slice 8 — resolved
decisions` section and are not restated here. The mapping from decision to PR is the delivery list
above and each slice's **Decisions** line below.

---

## Slices

All five are **behavior change** — fast RED-GREEN-REFACTOR increments, with the
mutation-or-alternate gate run once per PR at PR readiness. PR4 changes `api/` behaviour and
therefore carries a `scripts/test*.ts` wire-check against `vercel dev` + supabase (the epic's
routine-evidence line). PR3 carries a solo browser run. The rest are pure `src/core` + `src/ui`
and prove themselves at the unit layer.

**Required implementation skills:** `tdd`, `testing`, and `functional` throughout; `refactoring`
where a PR also tidies already-tested code; `front-end-testing` for PR3's terminal-observable
...
**Actor/trigger/outcome:** player runs `ldd <command>` on any box they stand on → one line per
linked library, `<lib>.so => /lib/<lib>.so (0x…)` when present or `not found` when the file is
gone; a command that links nothing prints legacy's not-a-dynamic-executable line.
**Class:** Behavior change.
**Decisions:** 67 (verbatim port, presence + paths, no version/CVE), 11 (`metadata.libraryLinks`
deleted).
**Path:** new `ldd` command in `src/core/commands/`, reading `libraryDeps` (already the authority)
and `env.fs` for each `/lib/<lib>.so`; registered in `registry.ts`. `ldd` is already stamped as a
binary (`binaries.ts:75`) with no command behind it, so this fills that stub — no packaging work.
...
- After `/lib/libpam.so` is removed, `ldd su` shows `libpam.so => not found` and still lists
  `libcrypt.so`.
- A command not in `libraryDeps` (e.g. `mkdir`) prints the not-a-dynamic-executable line, not an
  empty list.
...
### PR3 — a live library CVE escalates a shell on a box you hold (v0.237.0)
...
epic's PR3 evidence obligation): on a seeded box inside a window, `ldd su` then `msfconsole --local
...
### PR4 — the local exploit crosses to another player's box (v0.238.0)
...
that box is still open — mints a row at the granted tier and writes A's traces under A's writer key;
otherwise the uniform refusal.
**Class:** Behavior change.
...
nothing), 69 (server writes both trace lines under the owner's writer key), 70 (reuses slice 5's
...
and on that exact machine**; its tier is irrelevant (decision 66). The library interpreter from PR3
is the shared pure core both client and server call. An offline owner (no occupancy row) → `404
host_unreachable` → `No route to host`, nothing written.
**Acceptance:**
...
A's box, then A upgrades and B misses — with a negative control (dropping the open-session check
lets a non-occupant escalate, turning a check red).

---

### PR5 — the dependency map covers the game's own toolchain (v0.239.0)
...
**Actor/trigger/outcome:** player runs `ldd nmap` (or `node`/`hydra`/`gpg`/`lynx`) → real linked
libraries; `msfconsole --local nmap` inside a window → an effect from that command's own pool.
**Class:** Behavior change.
**Decisions:** 12 (map extends, eight libraries kept, thematic grouping intact), 68 (the extension
and its pool contents are this last PR, decided at its planning).
**Path:** add link entries to `libraryDeps` for the uncovered toolchain (`nmap`, `node`, `hydra`,
`gpg`, `lynx` and kin) — reusing the existing eight libraries, no new library, no `/lib` re-roll —
and a `SYSTEM_COMMAND_EFFECT_POOLS` entry per added command (a mapped command with no pool is not
exploitable, decision 12). **Open until this PR's planning:** the exact link set and each pool's
contents — in particular whether `node` gets a pool at all (a `node` CVE yielding `script_exec`
through the script runner is near-circular) or stays deliberately library-free like the shell
builtins. Shell builtins and UI commands (`cd`,`echo`,`pwd`,`whoami`,`clear`,`theme`) stay out.
...
- **The library tier floor is a real fork from services.** `exploitEffect.ts`'s `tierForSeverity`
  ...
  for `--local` would silently under-grant. PR3's interpreter must carry its own floor and a test
...
- **The server authorization is the whole PvP safety story (PR4).** `suElevate` never checks its
  parent session because a password is its authority; `--local` has none. If the open-session /
  signer / machine check is wrong, any signed player takes root on any online box. The wire-check's
  negative control exists for exactly this.

## Out of scope (recorded, not forgotten)

- Firmware (slice 9) — the third axis; needs PR3's per-axis interpretation shape but is its own
  slice.
