...
PHASE 1 — THE DOORS  (near-term focus)
  ...
  D10 polish (comfort commands + the whole long tail)  🚧 IN PROGRESS — 1 of 5 shipped, slice 2 planned
      D10 slice 1 the terminal is yours               ✅ SHIPPED v0.201.0 (#481)
      D10 slice 2 the card and the second window      📋 PLANNED — author overlay, fresh-tab xterm
...
**➡️ NEXT: D10 slice 2 — the card and the second window (`author`, `xterm`). 📋 PLANNED, not started.**
...
Four slices remain: the card and the second window (`author`, `xterm`) — planned — then the box's
own tools (`find`, `strings`), `chmod`, and `gpg`. Plan each of those when its predecessor lands. After D10 **Phase 1
...
**D10 slice 2 is planned** — [`d10-polish.md`](d10-polish.md) → "Slice 2", twelve acceptance
criteria confirmed 2026-09-02. It starts from the `CommandEnv` → UI capability seam
slice 1 laid (`clearScreen`, `currentTheme`, `setTheme` beside `resetGame`), which was the argument
for landing slice 1 first: `author` needs no capability at all — it is a `mode_change`, so it
travels the road `nano` and `lynx` already use — and `xterm` adds exactly one, `openTerminal`.
That makes five members of the "asks the UI to do something, gets nothing back" family, which is
where the grouped `env.ui.*` refactor comes up for judgement. Two of legacy's six unpainted theme
tokens (`link`, `avatarBorder`) arrive WITH the author card; the plan argues down a third. The
slice also pays off the two-tabs-one-session-stack hazard named in the grounding, because `xterm`
is the command that makes anyone hit it. No `api/` change anywhere in the door, so the wire-check
is `N/A` across all five slices and every close-out proof is a browser run.
