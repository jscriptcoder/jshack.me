// One-shot RLS verification for machine_filesystems.
//
...
const a1 = await anon.from('machine_filesystems').insert(row);
check(
...
const a2 = await anon.from('machine_filesystems').select('*');
check(
...
// Cleanup
await sr.from('machine_filesystems').delete().eq('machine_id', 'rls-probe');
